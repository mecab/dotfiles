...
if type kubectl >/dev/null 2>&1; then
    ...
    alias ktx='kubectl config use-context $(kubectl config get-contexts -o name | fzf --height=50%)'
    alias kns='kubectl config set-context --current --namespace=$(kubectl get namespaces --no-headers -o custom-columns=':metadata.name' | fzf --height=50%)'
